...
/**
 * Read the repository shape into a catalog: every directory under `src/tools/` is
...
export function readCatalog(root = repoRoot) {
  return readDirectories(path.join(root, "src", "tools")).map((category) => {
    const tools = readDirectories(path.join(root, "src", "tools", category))
      .filter((tool) => hasReadme(root, "src", "tools", category, tool))
      .map((tool) => ({
        category,
        tool,
        readme: `src/tools/${category}/${tool}/README.md`,
        platforms: readDirectories(
          path.join(root, "src", "tools", category, tool, "examples"),
        ).filter((platform) =>
          hasReadme(root, "src", "tools", category, tool, "examples", platform),
        ),
      }));
    ...
    for (const { tool, platforms, readme } of tools) {
      ...
      for (const platform of platforms) {
        pages.push({
          ...
          source: `src/tools/${category}/${tool}/examples/${platform}/README.md`,
          category,
          tool,
          platform,
        });
