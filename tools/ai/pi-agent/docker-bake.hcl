# Variants of pi-agent. Run from this directory:
#   docker buildx bake                    # build every variant
#   docker buildx bake pi-agent-ubuntu   # build one variant
...
target "pi-agent" {
  ...
  context = "."
  tags    = ["tool-containers/pi-agent:${v.variant}"]
  args = {
    BASE_IMAGE = v.base
  }
  labels = {
    "org.opencontainers.image.title"         = "pi-agent"
    "org.opencontainers.image.description"   = "Pi coding agent on devbox"
    "org.opencontainers.image.documentation" = "https://github.com/hambn/tool-containers/tree/main/tools/ai/pi-agent"
