...
install_zsh_autosuggestions() {
    ...
    zsh_autosuggestions_tag="${ZSH_AUTOSUGGESTIONS_VERSION:-latest}"
    if [ "$zsh_autosuggestions_tag" = latest ]; then
    ...
    printf '%s\n' "${zsh_autosuggestions_tag#v}" \
        >/usr/local/share/workspace/zsh-autosuggestions.version
}
...
install_helm() {
    ...
    helm_tag="${HELM_VERSION:-latest}"
    if [ "$helm_tag" = latest ]; then
    ...
    printf '%s\n' "${helm_tag#v}" >/usr/local/share/workspace/helm.version
}
...
install_kustomize() {
    ...
    kustomize_tag="${KUSTOMIZE_VERSION:-latest}"
    if [ "$kustomize_tag" = latest ]; then
    ...
    printf '%s\n' "${kustomize_tag#v}" >/usr/local/share/workspace/kustomize.version
}
...
install_yq() {
    ...
    yq_tag="${YQ_VERSION:-latest}"
    if [ "$yq_tag" = latest ]; then
    ...
    rm -f /tmp/yq
    printf '%s\n' "${yq_tag#v}" >/usr/local/share/workspace/yq.version
}
