...
workspace is the base image family (Ubuntu and Alpine, with optional headless Chromium) the tool images build on. This page runs it on Podman with copy-paste examples; every file in this directory is shown below exactly as it exists in the repository.
...
Save as `~/.config/containers/systemd/workspace.container`, then run `systemctl --user daemon-reload && systemctl --user start workspace`:
