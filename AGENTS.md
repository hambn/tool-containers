...
`origin/main`, then finish the task by committing, pushing the branch, opening a pull
request to `main`, resolving its review feedback, and squash-merging it yourself once
checks pass. This default lets multiple agents run at the same time without interrupting
each other; the guide defines the narrow work-in-place and local-only exceptions.
