...
- This repository is a monorepo of containerized tools organized as
...
- Current catalog entries include `images/ai/claude-code`, `images/ai/t3code`,
  `images/ai/codex`, `images/ai/open-code-review`, `images/ai/pi-agent`,
  `images/ai/agentbloat`, `images/ai/omnigent`, and `images/base/agentimg`.
- `images/ai/codex` publishes four Codex CLI profiles directly on the matching `agentimg`
  tags, with `ubuntu-browser` as primary.
- `images/ai/open-code-review` publishes four OCR profiles on the matching `agentimg` tags;
...
- Reusable foundation images belong in the `images/base/` category; user-facing tools
  remain in their purpose-specific categories.
- `images/base/agentimg` publishes four variants: `ubuntu-browser` (primary), `ubuntu`,
  `alpine-browser`, and `alpine`. It contains no AI agents or Exe-specific components;
  its nginx and Ghostty support are generic development facilities without Exe content.
...
- Pi source edits and `agentimg` base refreshes repoint moving tags only; a detected
  ...
  image.
- Every `agentimg` variant defaults to the UID/GID-1000 `sysadmin` user and a styled Zsh
  login shell. Derived image Dockerfiles switch to root only for build-time installation
  and restore `USER sysadmin` before their runtime contract.
- Agentimg owns the shared Node.js runtime for downstream AI images. Derived images do
  not reinstall Node and accept digest-pinned base images through `BASE_IMAGE`.
- Repository automation pins third-party actions to immutable commits, publishes SBOM
  and provenance attestations, blocks on HIGH/CRITICAL Trivy findings, and uses
  least-privilege job permissions.
- Pull requests use Conventional Commit titles, Summary and Validation sections,
  CODEOWNERS, dependency review, and path-based labeling. Issue forms and private
  vulnerability reporting provide structured support channels.

## Change records

### 2026-08-21 — Port repository hardening and add GitHub controls

- Scope: transplanted the useful security, CI, image, and deployment changes from
  `agent/repository-hardening` onto the current `images/` layout; added repository
  validation, Dependabot grouping, PR policy and label workflows, dependency review,
  CODEOWNERS, contribution/security guidance, and structured PR and issue templates.
- Decision: preserve the current `sysadmin` and modular agentimg contracts, pin external
  actions, fail closed on unsafe publication and image scans, and use solo-maintainer
  PR controls without mandatory approvals.
- Validation: `.github/scripts/validate-repository.sh` passed workflow and shell parsing,
  Dockerfile contracts, Markdown links, Compose rendering, and Helm lint/render;
  `bash .agents/commands/check-agent-workspace.sh`, explicit runtime-user assertions,
  and `git diff --check` also passed. No local image build was run.

### 2026-08-21 — Modularize agentimg Zsh with selected Oh My Zsh sources

- Scope: changed the shared agentimg `.zshrc` into an XDG `conf.d` and plugin loader,
  added image-owned option, alias, and prompt overrides, and taught both distro Zsh
  setup scripts to sparse-fetch Oh My Zsh libraries plus only its Git and kubectl
  plugins.
- Decision: install Oh My Zsh `lib/*.zsh` directly into `~/.config/zsh/conf.d`, retain
  independently installed autosuggestions and syntax highlighting, and keep syntax
  highlighting last in startup order.
- Validation: local builds and tests were not run per explicit request; the edited
  sources were inspected through the working-tree diff only.

### 2026-08-21 — Split agentimg setup and rename its runtime user

- Scope: replaced the monolithic Ubuntu and Alpine setup scripts with focused scripts
  under distro-local `scripts/` directories, used separate Dockerfile `RUN` layers,
  and renamed the UID/GID-1000 runtime account from `agent` to `sysadmin` across
  agentimg, downstream Dockerfiles, documentation, and CI smoke tests.
- Decision: keep build contexts self-contained, isolate browser-only packages, and
  restore `USER sysadmin` after build-time installation in every derived image.
- Validation: shell syntax, Dockerfile references, runtime-user consistency, workflow
  checks, and `git diff --check` passed; no local image build was run.
